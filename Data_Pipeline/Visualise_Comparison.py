...
# Input Paths
BMTGNN_DIR = PROJECT_ROOT / "Processed_Data" / "B-MTGNN"
VISION_DIR = PROJECT_ROOT / "Processed_Data" / "visionpp"
LEGACY_CSV_PATH = PROJECT_ROOT / "B-MTGNN" / "data" / "sm_data_g.csv"

# Output Paths
OUTPUT_DIR = PROJECT_ROOT / "Processed_Data" / "Comparison_Plots"
BMTGNN_OUT = OUTPUT_DIR / "BMTGNN_Outlook"
VISION_OUT = OUTPUT_DIR / "VisionPP_Outlook"
...
def generate_comparisons():
    ...
    # 1. Load B-MTGNN Data
    try:
        b_preds = np.load(BMTGNN_DIR / 'predictions.npy').squeeze() 
        b_conf = np.load(BMTGNN_DIR / 'confidence.npy').squeeze()   
        b_hist = np.load(BMTGNN_DIR / 'history_data.npy')           
        
        df_legacy = pd.read_csv(LEGACY_CSV_PATH, nrows=0)
        b_names = df_legacy.columns.tolist()[1:] # Exclude 'Date'
