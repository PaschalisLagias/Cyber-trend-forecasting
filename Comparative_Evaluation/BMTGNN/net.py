...
class gtnet(nn.Module):
    def __init__(
        self,
        gcn_true, buildA_true, gcn_depth, num_nodes, device, predefined_A=None,
        static_feat=None, dropout=0.3, subgraph_size=20, node_dim=40,
        dilation_exponential=1, conv_channels=32, residual_channels=32,
        skip_channels=64, end_channels=128, seq_length=12, in_dim=2, out_dim=12,
        ...
        #   residual_channels, out_channels=residual_channels, num_heads=1
        # )
    ...
    def forward(self, input_, idx=None):
        ...
        if self.gcn_true:
            if self.buildA_true:
                if idx is None:
                    ...
                    # this line computes the adjacency matrix adaptively by
                    # calling the function forward in the gc
                    self.adp = self.gc(self.idx)
                else:
        ...
        skip = self.skip0(F.dropout(
            input_, self.dropout, training=self.training
        ))
        ...
        for i in range(self.layers):
            ...
            if self.gcn_true:
                ...
                x2, attention_matrix2 = \
                    self.gconv2[i](x, self.adp.transpose(1, 0))

                x = x1 + x2

                # Combine attention matrices
                attention_matrix = (attention_matrix1 + attention_matrix2) / 2

                # Save attention scores
                self.attention_scores = attention_matrix
            else:
    ...
    def compute_saliency(self, input_, time_step, node_idx, time=False):
        ...
        # Assuming a shape like (sequence_length, features)
        output[time_step, node_idx].backward()
        print(
            f'output[{time_step}, {node_idx}] = {output[time_step, node_idx]}'
        )

        # Get the gradients of the input
        gradient = input_.grad.clone()

        # Make sure to zero out gradients if you plan to reuse the input
        input_.grad.zero_()
    ...
    def explain_by_adjacency(self, node_idx, input_):
        ...
        for j in range(self.adp.shape[1]):
            ...
            if self.adp[node_idx, j].item() > 0:
                ...
                # this will allow us to have adjacent node, adjacency weight,
                # and adjacent node value in the input
                E[col[j]] = [self.adp[node_idx, j].item(), input_[j].item()]

        print(E)        
        print(len(E))

    def explain_subgraph_by_adjacency(self):
        col = DataLoaderS.col
    ...
    def visualize_sample_attention_scores(self):
        """
        Define the visualize_attention_scores function for a small sample.
        """
    ...
    def visualize_attention_scores(self, names, rows, columns, title):
        ...
        if len(col_labels1) != attention_scores.shape[0]:
            error_msg = "Length of col_labels should match the subgraph size"
        ...
        heatmap=sns.heatmap(
            attention_scores.cpu().numpy(), cmap='OrRd', annot=False,
        ...
        # pyplot.xlabel("Nodes")
        # pyplot.ylabel("Nodes")
        pdf_path = 'Attention_' + title + '.pdf'
        pyplot.savefig(pdf_path, format='pdf', bbox_inches="tight")
        pyplot.show()
    ...
    def visualize_filters(self):
        ...
        for i, weights in enumerate(conv_weights):
            ...
            for j in range(num_filters):
                heatmap = axes[j].imshow(
                    weights[j, 0].cpu().numpy(), cmap='viridis',
    ...
    def visualise_data(self, t1, t2, label1, label2):
        ...
        sns.heatmap(
            t1.detach().unsqueeze(0).numpy(), cmap='YlGnBu', annot=False,
            fmt=".2f", xticklabels=month_year_labels,yticklabels=False
        )
        ...
        sns.heatmap(
            t2.detach().unsqueeze(0).numpy(), cmap='YlGnBu', annot=False,
            fmt=".2f", xticklabels=month_year_labels,yticklabels=False
        )
        ...
        # Show the plot
        pyplot.tight_layout()
        pdf_path = 'Data_'+label1+'_'+label2+'.pdf'
        pyplot.savefig(pdf_path, format='pdf', bbox_inches="tight")
        pyplot.show()
