...
def inverse_diff_2d(output, init_value, shift):
    output[0, :] = torch.exp(
        output[0, :] + torch.log(init_value + shift)
    ) - shift
...
def inverse_diff_3d(output, init_value, shift):
    output[:, 0, :] = torch.exp(
        output[:, 0, :] + torch.log(init_value + shift)
    ) - shift
...
def plot_data(data, title):
    ...
    fig = plt.gcf()
    # plt.savefig(
    # 'model/comparison/auto141/' + title[0:4] + str(counter+1) + '.png',
    # bbox_inches="tight"
    # )
...
###############################################################################
    ...
    # Relative Absolute Error RAE
    # absolute the result and sum over all elements
...
def plot_predicted_actual(
    ...
    for year in range(11, 23):
        for month in months:
            exckuded_months = {'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'}
            if year == 11 and month not in exckuded_months:
                continue
    ...
    if type_ == 'Testing':
        ...
        for index, value in enumerate(M):
            if any(m in M[index] for m in ('Dec', 'Mar', 'Jun', 'Sep')):
                M2.append(M[index])
                p.append(index + 1)
    ...
    # plt.fill_between(
    # x, predicted-variance.numpy(), predicted+variance.numpy(),
    # alpha=0.1, color='purple', label='Variance')
    ...
    # PNG
    png_path = type_ + '/' + title + '_' + type_ + '.png'
    plt.savefig(png_path, bbox_inches="tight")

    # PDF
    pdf_path = type_ + '/' + title + '_' + type_ + ".pdf"
...
def s_mape(y_true, y_pred):
    ...
    for i in range(len(y_true)):
        mape += abs(y_true[i] - y_pred[i]) / (abs(y_true[i]) + abs(y_pred[i]))
    mape /= len(y_true)
    return mape
...
def evaluate_sliding_window(
    ...
    # X.shape[0]=n_input+36 (as example)?
    for i in range(n_input, test_window.shape[0], data.out_len):

        print('**************x_input*******************')
        # scale will have the max of each column (142 max values)
        # scale = data.scale.expand(x_input.size(0), data.m)
        ...
        for _ in range(num_runs):
            with torch.no_grad():
                ...
                # if this is the last predicted window
                # it exceeds the test window range
        ...
        if data.P <= data.out_len:
            ...
            x_input = torch.cat(
                [x_input[-(data.P-data.out_len):, :].clone(), y_pred.clone()],
                dim=0
            )

        # scale will have the max of each column (142 max values)
        # scale = data.scale.expand(y_pred.size(0), data.m)
        ...
        # total_loss += evaluate_l2(y_pred, y_true).item()  # MSE
        # total_loss_l1 += evaluate_l1(y_pred, y_true).item()  # MAE
...
###############################################################################
    ...
    if is_plot:
        for v in range(r, r + 142):
            ...
            #     predict[:, col],
            #     y_test[:, col],
            #     f"Testing node {col}, RRSE = {round(rrse, 2)},"
            #     f" CORR = {round(correlation, 2)}",
            #     counter,
            # )

            node_name = DataLoaderS \
                .col[col] \
                .replace('-ALL', '') \
            ...
            # plot
            plot_predicted_actual(
                predict[:, col], y_test[:, col],
                node_name, 'Testing',
                variance[:, col], confidence_95[:, col]
            )
...
def train(data, X, Y, model, criterion, optim, batch_size):
    ...
    for X, Y in data.get_batches(X, Y, batch_size, True):
        ...
        if iter % 1 == 0:
            ...
            print(f"iter:{iter:3d} | loss: {loss.item() / denom:.3f}")
...
def main(experiment):
    ...
    for q in range(30):
        ...
        # print(data_.shift)
        # for i in range(data_.dat.shape[1]):
        #     plot_data(data_.dat[:,i],'Node '+str(i))
        # sys.exit()     

        print('train X:', data_.train[0].shape)
        print('train Y:', data_.train[1].shape)
        print('valid X:', data_.valid[0].shape)
        print('valid Y:', data_.valid[1].shape)
        print('test X:', data_.test[0].shape)
        print('test Y:', data_.test[1].shape)
        print('test window:', data_.test_window.shape)

        print('length of training set=', data_.train[0].shape[0])  # Zaid
        print('length of validation set=', data_.valid[0].shape[0])  # Zaid
        print('length of testing set=', data_.test[0].shape[0])  # Zaid
        print('valid=', int((0.43 + 0.3) * data_.n))
        
        model = gtnet(
            args.gcn_true, args.buildA_true, gcn_depth, args.num_nodes, device,
            data_.adj, dropout=dropout, subgraph_size=k, node_dim=node_dim,
        ...
        # model = net_lstm(
        # 2, 40, args.gcn_true, args.buildA_true, gcn_depth, args.num_nodes,
        # device, data_.adj, None, dropout=dropout, subgraph_size=k,
        # node_dim=node_dim, seq_length=args.seq_in_len, in_dim=args.in_dim,
        # out_dim=args.seq_out_len, layers=layer, propalpha=prop_alpha,
        # tanhalpha=tanh_alpha, layer_norm_affline=True
        # )
        ...
        n_params_ = sum([p.nelement() for p in model.parameters()])
        print('Number of model parameters is', n_params_, flush=True)
        ...
        # best_val = 10_000_000
        optim = Optim(
            model.parameters(), args.optim, lr,
        ...
        try:
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                print('Iter:', q)
                print('epoch:', epoch)
                print('hp=', [gcn_depth, lr, conv, res, skip, end,
                ...
                train_loss = train(
                    data_, data_.train[0], data_.train[1], model,
                    criterion, optim, args.batch_size
                )

                val_loss, val_rae, val_corr, val_smape = evaluate(
                    data_, data_.valid[0], data_.valid[1], model, evaluate_l2,
                    evaluate_l1, args.batch_size, False
                )

                # val_loss, val_rae, val_corr, val_smape = \
                # evaluate_sliding_window(
                # data_, data_.test_window, model, evaluate_l2,
                ...
                # Save model if validation loss is the best we've seen so far.
                ...
                if (not math.isnan(val_corr)) and val_loss < best_rse:
                    ...
                    best_hp = [gcn_depth, lr, conv, res, skip, end, k,
                               dropout, dilation_ex, node_dim, prop_alpha,
                               tanh_alpha, layer, epoch
                               ]
                    
                    es_counter = 0

                    # test_acc, test_rae, test_corr = evaluate(
                    # data_, data_.test[0], data_.test[1], model,
                    # evaluate_l2, evaluate_l1, args.batch_size
                    # )
    ...
    vtest_acc, vtest_rae, vtest_corr, vtest_smape = evaluate(
        data_, data_.valid[0], data_.valid[1], model,
        evaluate_l2, evaluate_l1, args.batch_size, False
    )

    # vtest_acc, vtest_rae, vtest_corr, vtest_smape = evaluate_sliding_window(
    # data_, data_.test_window, model, evaluate_l2,
    # evaluate_l1, args.seq_in_len, True
    # )
