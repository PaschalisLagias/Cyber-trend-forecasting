...
def load_visualisation_data():
    ...
    It retrieves:
    1. Model predictions (predictions.npy).
    2. Column names (node_names.npy).
    ...
    graph_csv_path = config.project_root / "B-MTGNN" / "data" / "graph.csv"
    ...
    if not success:
        trues = np.load(data_dir / 'ground_truth.npy')
        if trues.ndim == 4: trues = trues.squeeze(-1)
        history_data = trues[-1, :, :]

    trues_period = np.load(data_dir / 'ground_truth.npy')
...
def perform_broad_analysis(data):
    ...
    ax.plot(horizon, avg_pred, color='blue', linestyle='--', marker='x', label='PDFormer Forecast')
